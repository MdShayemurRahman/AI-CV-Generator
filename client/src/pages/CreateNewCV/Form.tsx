...
const Form = () => {
  ...
  const handleSubmitCV = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post('/api/resume' /* yourFormData */);
      console.log('Resume created:', response.data);
      // Handle successful resume creation or navigate to another page
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <Grid display='flex' justifyContent='right' padding={3}>
        <Button onClick={()=> handleSubmitCV} type='submit' variant='contained'>
          Submit
        </Button>
      </Grid>
    </Box>
